...
export async function scrapePlayers() {
  const playersInfo = { players: [] as any[] };
  // Fetch the data
...
export async function scrapeTeams(region: string) {
  const teamsInfo = { teams: [] as any[] };
  ...
  $(item)
    ...
    .each((index, element) => {
      teamsInfo.teams.push({
        ...
        rating_score: $(element).find("div.rank-item-rating").eq(0).text().trim(),
        recent_match: {
          match_url: `www.vlr.gg` + $(element).find("a.rank-item-last").attr("href"),
          match_time: $(element).find("a.rank-item-last div").eq(0).text().trim(),
          opponent_team_name: $(element).find("a.rank-item-last div span").eq(1).text().trim(),
          opponent_team_logo_url: $(element).find("a.rank-item-last div img").attr("src"),
        },
...
export async function scrapeEvents() {
  const eventsInfo = { events: [] as any[] };
...
export async function scrapeEvent(event_url: string) {
  const eventInfo = {
    ...
    location: "",
    teams: [] as any[],
    upcoming_matches: [] as any[],
    latest_results: [] as any[],
  };
  ...
  $(item)
    ...
    .each((index, element) => {
      eventInfo.teams.push({
        ...
        team_logo_url: $(element).find("div.event-team-players img").attr("src"),
      });
    });
  ...
  $(item)
    ...
    .each((index, element) => {
      eventInfo.upcoming_matches.push({
        team_one_name: $(element).find("div.event-sidebar-matches-team div.name span").eq(0).text().trim(),
        team_two_name: $(element).find("div.event-sidebar-matches-team div.name span").eq(1).text().trim(),
  ...
  $(item)
    ...
    .each((index, element) => {
      eventInfo.latest_results.push({
        team_one_name: $(element).find("div.event-sidebar-matches-team div.name span").eq(0).text().trim(),
        team_one_score: $(element).find("div.event-sidebar-matches-team div.score").eq(0).text().trim(),
        team_two_name: $(element).find("div.event-sidebar-matches-team div.name span").eq(1).text().trim(),
        team_two_score: $(element).find("div.event-sidebar-matches-team div.score").eq(1).text().trim(),
        ETA: $(element).find("div.eta").text().trim(),
      });
    });
      ...
      $(element)
        ...
        .each((ind, ele) => {
          upcomingMatches.matches.push({
            team_one_name: $(ele).find("div.match-item-vs-team-name").eq(0).text().trim(),
            team_two_name: $(ele).find("div.match-item-vs-team-name").eq(1).text().trim(),
            match_url: `www.vlr.gg` + $(ele).attr("href"),
            event_name: $(ele).find("div.match-item-event").clone().children().remove().end().text().trim(),
...
export async function scrapeMatchResults() {
  const matchResults = { matches: [] as any[] };
      ...
      $(element)
        ...
        .each((ind, ele) => {
          matchResults.matches.push({
            team_one_name: $(ele).find("div.match-item-vs-team-name").eq(0).text().trim(),
            team_two_name: $(ele).find("div.match-item-vs-team-name").eq(1).text().trim(),
            team_one_score: $(ele).find("div.match-item-vs-team-score").eq(0).text().trim(),
            team_two_score: $(ele).find("div.match-item-vs-team-score").eq(1).text().trim(),
            match_url: `www.vlr.gg` + $(ele).attr("href"),
            event_name: $(ele).find("div.match-item-event").clone().children().remove().end().text().trim(),
            event_icon_url: $(ele).find("div.match-item-icon img").attr("src"),
            match_time: $(ele).find("div.match-item-time").text().trim(),
...
export async function scrapeMatch(match_url: string) {
  const matchInfo = {
    tournament_name: "",
    tournament_logo: "",
    match_url: "",
    ...
    team_two_logo: "",
    team_two_score: "",
    team_one_players: [] as any[],
    team_two_players: [] as any[],
  };
  ...
  matchInfo.tournament_name = $(item).find("a.match-header-event  div div").eq(0).text().trim();
  matchInfo.tournament_logo = $(item).find("a.match-header-event  img").attr("src") || "";
  matchInfo.match_url = match_url;
  matchInfo.match_day = $(item).find("div.moment-tz-convert").eq(0).text().trim();
  matchInfo.team_one_name = $(item).find("a.mod-1 div.wf-title-med").text().trim();
  matchInfo.team_one_logo = $(item).find("a.mod-1 img").attr("src") || "";
  matchInfo.team_one_score = $(item).find("div.match-header-vs-score div span").eq(0).text().trim();
  matchInfo.team_two_name = $(item).find("a.mod-2 div.wf-title-med").text().trim();
  matchInfo.team_two_logo = $(item).find("a.mod-2 img").attr("src") || "";
  matchInfo.team_two_score = $(item).find("div.match-header-vs-score div span").eq(2).text().trim();
  ...
  $(item)
    ...
    .each((index, element) => {
      ...
      matchInfo.team_one_players.push({
        ...
        kill_assist_trade_survive_percentage: $(tds[8]).find("span span").eq(0).text().trim(),
        average_damage_per_round: $(tds[9]).find("span span").eq(0).text().trim(),
        ...
        first_kills_first_deaths: $(tds[13]).find("span span").eq(0).text().trim(),
      });
    });
